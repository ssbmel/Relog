...
export function HeroSection() {
  return (
    <section className="relative flex flex-col items-center justify-center overflow-auto px-6 pt-20 pb-8 text-center h-full">
      {/* 뱃지 */}
      ...
      <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row">
        <Button asChild size="lg" className="h-10 rounded-full px-7 text-sm shadow-lg shadow-primary/20">
          <Link href="/signup">
            무료로 시작하기
            <ArrowRight className="ml-2 h-4 w-4" />
          </Link>
        </Button>
        <Button asChild variant="outline" size="lg" className="h-10 rounded-full px-7 text-sm bg-transparent">
        ...
        <div className="rounded-xl border border-border bg-card/80 shadow-2xl shadow-black/10 backdrop-blur overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-2 px-3 pb-3">
            {[
              ...
              <div key={c.name} className="flex items-center gap-2 rounded-lg border border-border bg-background/60 px-2.5 py-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[9px] text-muted-foreground truncate">{c.company}</p>
                </div>
                <span className="ml-auto text-[9px] text-muted-foreground shrink-0">{c.date}</span>
              </div>
            ))}
